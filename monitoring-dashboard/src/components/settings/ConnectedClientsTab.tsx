  ...
  const handleSaveName = async (clientId: string) => {
    ...
    try {
      await api.put(`/connected-clients/${clientId}/name`, {
        ...
        employeeId: editId.trim() || undefined,
      });
      
      // Update local state
      setClients(clients.map(c => 
        c.clientId === clientId 
          ? { ...c, employeeName: editName.trim(), employeeId: editId.trim() || null }
          : c
      ));
      ...
      toast.error(err.response?.data?.error || 'Failed to update client information');
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                    {editingClientId === client.clientId ? (
                      <input
                        ...
                        className="px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white w-full"
