...
; Create scheduled task with working directory set to installation folder
; The /create command doesn't support setting working directory directly, so we use XML
...
begin
  ...
  begin
    // Create XML file for scheduled task with working directory
    ...
    TaskXML := '<?xml version="1.0" encoding="UTF-16"?>' + #13#10 +
               ...
               '    <Exec>' + #13#10 +
               '      <Command>' + ExpandConstant('{app}\VibgyorSeekMonitoring.exe') + '</Command>' + #13#10 +
